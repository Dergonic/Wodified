...
// COMPTEUR */
// Compte le nombre d'éxécution du script.
//	REM : Utilise un fichier texte "compteur.txt" => droits d'écriture sur le FTP 
...
$classes_feminin = array( "Alchimiste", "Archère", "Barbare", "Barde", "Chamane", "Chasseresse", "Femme chevalier", "Erudite", "Gladiatrice", "Jongleuse", "Paladine", "Prêtresse", "Risque-tout", "Sorcière");
...
$races_feminin = array( "Dinturane", "Elfe Mag-Mor", "Elfe Tirem-Ag", "Frontalière", "Gnome", "Femme des bois", "Kérasie", "Naine des collines", "Naine des montagnes", "Rashanie", "Semie-femme");
...
if (isset($_POST['matos']) and !empty($_POST['matos'])) {
	...
	// Etape 1.1 Equipement (Objets pris)
	...
	$raw = substr($raw, $pos1); //On enleve tout ce qu'il y a avant le sac (donc les "objets pris")
	...
	if (!$pos1) { // Si pas de décoration, on cherche le lien "[cacher]"/"[afficher]" des "indications" qui est juste après
		$bln_décorations = false;
		$pos1 = strpos($raw,'[Cacher]');
		if (!$pos1) 
			$pos1 = strpos($raw,'[Afficher]');
	}
	...
	$raw = substr($raw, $pos1); // on enlève les données traitées
	
	
	// Etape 1.3 Décorations
	if($bln_décorations) {
		$pos = 0;
		$pos1 = strpos($raw,'[Cacher]');
		if (!$pos1) 
...
// ETAPE 3 : "Les attributs"
if (!$msg_err && isset($_POST['attributes']) and !empty($_POST['attributes'])) {
	$raw = trim(stripslashes($_POST['attributes']));
	$line = substr($raw,strpos($raw,'Quitter'),strpos($raw,"Dans le forum")); //Dergonic : Second param doit etre "longueur" => doute sur la strpos .... ET "Dans le Forum est AVANT Quitter


	if (!empty($_POST['monde']) and in_array($_POST['monde'],$mondes)) {//Dergonic : Champ commenté dans le HTML => $_POST['monde'] n'existe pas
	...
	if (strstr($m,' ')) //Dergonic : $m inconnu à cet endroit ! Instruction inutile
		$out['monde'] = '';
		
	$line=$raw;
	
	$line=str_replace($classes_feminin, $classes, $line);
	$line=str_replace($races_feminin, $races, $line);
	...
	/*NF Auto identification du nom (ok avant)*/
	if (!empty($_POST['name'])) {
	...
	// Etape 3.1 : Récupération du Titre
	...
	// Etape 3.2 : Récupérations des attributs
	$pos = strpos($raw,"\r\nAttribut");
	...
	// Etape 3.3 : Récupération des informations autres (PV, PM, ...)
	...
	// Etpae 3.4 : Récupération des Armures
...
// ETAPE 4 : Calcul de divers bonus
// Bonus Equip
...
if (isset($_POST['talents']) and !empty($_POST['talents'])) {
	...
	// Récupération du tableau des talents
	...
	foreach ($match[1] as $n=>$val) {
		if (strpos($val,'partir du niveau')) // Sorts pas encore disponibles (trop faible niveau)
			break;
...
<?php } else { // Pas de fichier généré. Affichage du formulaire
